...
import { Link } from '@/src/lib/router-shim';
          ...
          <section className="grid gap-6 xl:grid-cols-[260px_1fr_300px]">
            ...
            <section className="min-w-0">
              <div className="flex items-end justify-between gap-4 mb-4">
                ...
                <Link to="/admin/science" className="inline-flex items-center gap-2 text-sm text-red-400 hover:text-red-300">
            ...
            <aside className="space-y-4">
              <Panel title="Continue Learning" icon={Target}>
                {enrolledCourses.length === 0 ? (
                  ...
                  return (
                    <Link key={course.id} to={`/science-tech/courses/${course.slug}`} className="block border-b border-gray-800 pb-3 last:border-b-0 last:pb-0">
              ...
              <Panel title="Reference Library" icon={FileText}>
                {textbooks.length === 0 ? (
                  ...
                  <Link key={book.id} to={`/science-tech/textbooks/${book.id}`} className="block border-b border-gray-800 pb-3 last:border-b-0 last:pb-0">
...
const CourseRow = ({ course, user }) => {
  ...
  return (
    <Link to={`/science-tech/courses/${course.slug}`} className="grid md:grid-cols-[1fr_90px_90px_100px_120px_36px] gap-3 px-4 py-4 border-b border-gray-800 last:border-b-0 hover:bg-red-950/10 transition-colors">
...
const CourseCompactCard = ({ course, user }) => {
  const enrollment = (course.science_enrollments || []).find((item) => item.user_id === user?.id);
  return (
    <Link to={`/science-tech/courses/${course.slug}`} className="border border-gray-800 bg-black/30 p-4 hover:border-red-900/60">
