...
import { useNavigate, useSearchParams } from '@/src/lib/router-shim';
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (coverFile) {
                ...
                coverImageUrl = `${(process.env.NEXT_PUBLIC_SUPABASE_URL || process.env.REACT_APP_SUPABASE_URL)}/storage/v1/object/public/covers/${encodeURIComponent(coverFilename)}`;
            ...
            const token = getStoredToken();
            const anonKey = (process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY || process.env.REACT_APP_SUPABASE_ANON_KEY);
            const restBase = `${(process.env.NEXT_PUBLIC_SUPABASE_URL || process.env.REACT_APP_SUPABASE_URL)}/rest/v1/digital_library_books`;
            ...
            if (oldPdfToRemove || oldCoverToRemove) {
                try {
                    if (oldPdfToRemove) {
                        fetch(`${(process.env.NEXT_PUBLIC_SUPABASE_URL || process.env.REACT_APP_SUPABASE_URL)}/storage/v1/object/library/${encodeURIComponent(oldPdfToRemove)}`, {
                            method: 'DELETE', headers: { 'Authorization': `Bearer ${token}` },
                        }).catch(() => {});
                    }
                    if (oldCoverToRemove) {
                        fetch(`${(process.env.NEXT_PUBLIC_SUPABASE_URL || process.env.REACT_APP_SUPABASE_URL)}/storage/v1/object/covers/${encodeURIComponent(oldCoverToRemove)}`, {
                            method: 'DELETE', headers: { 'Authorization': `Bearer ${token}` },
                        }).catch(() => {});
                    }
                } catch (e) {
