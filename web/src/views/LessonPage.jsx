...
import { Link, useNavigate, useParams } from '@/src/lib/router-shim';
...
const LessonPage = () => {
  ...
  const completeLesson = async () => {
    if (!data?.lesson || !user) {
      navigate('/login', { state: { from: `/science-tech/courses/${courseSlug}/${chapterSlug}/${lessonSlug}` } });
      return;
    }
    ...
    try {
      if (data.demoMode && !user) {
        if (nextLesson) {
          navigate(`/science-tech/courses/${courseSlug}/${nextLesson.moduleSlug}/${nextLesson.lessonSlug}`);
        } else {
          navigate(`/science-tech/courses/${courseSlug}`);
        }
        return;
      }
    ...
    return (
      <div className="min-h-screen bg-[#12131A]">
        ...
        <main className="pt-24 px-4 text-center">
          ...
          <Link to={`/science-tech/courses/${courseSlug}`} className="mt-4 inline-flex text-red-400 hover:text-red-300">
            Back to course
          </Link>
        </main>
      </div>
        ...
        <div className="border-b border-gray-800 bg-black/35">
          <div className="container mx-auto max-w-7xl px-4 py-4">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <div>
                <Link to={`/science-tech/courses/${courseSlug}`} className="inline-flex items-center gap-2 text-gray-500 hover:text-white text-sm mb-2">
        ...
        <div className="container mx-auto max-w-7xl px-4 py-6 grid gap-6 xl:grid-cols-[280px_minmax(0,1fr)_280px]">
                  ...
                  <div key={module.id} className="border-b border-gray-800/60 last:border-b-0">
                    ...
                    {(module.science_lessons || []).map((lesson, lessonIndex) => {
                      ...
                      return (
                        <Link
                          key={lesson.id}
                          to={`/science-tech/courses/${courseSlug}/${module.slug}/${lesson.slug}`}
          ...
          <section className="space-y-5 min-w-0">
            ...
            <div className="border-t border-gray-800 pt-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              {previousLesson ? (
                <Link to={`/science-tech/courses/${courseSlug}/${previousLesson.moduleSlug}/${previousLesson.lessonSlug}`} className="inline-flex items-center gap-2 text-gray-400 hover:text-white">
              ...
              <div className="flex flex-wrap gap-3">
                {nextLesson && (
                  <Link to={`/science-tech/courses/${courseSlug}/${nextLesson.moduleSlug}/${nextLesson.lessonSlug}`} className="inline-flex items-center gap-2 border border-gray-800 px-4 py-2 text-gray-300 hover:border-gray-600 hover:text-white">
                    Next lesson
          ...
          <aside className="hidden xl:block">
            <div className="sticky top-24 space-y-4">
              ...
              <Panel title="Checkpoint">
                <Link to={`/science-tech/courses/${courseSlug}/${chapterSlug}/test`} className="inline-flex items-center justify-between w-full border border-gray-800 px-3 py-2 text-gray-300 hover:border-red-900/60 hover:text-white">
                  Module checkpoint
                  <ChevronRight className="w-4 h-4" />
                </Link>
              </Panel>
