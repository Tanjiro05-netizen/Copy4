        ...
        {error && !loading && filteredPosts.length === 0 ? (
          ...
          <>
            <section className={s.heroGrid} aria-label="Featured Substack article">
              <Link to={`/substack/${featuredPost.slug}`} className={s.featuredStory}>
                {featuredPost.imageUrl ? (
                  <img src={featuredPost.imageUrl} alt="" className={s.featuredImage} />
                ) : (
              ...
              <aside className={s.sideRail} aria-label="Recent Substack articles">
                {sidePosts.map((post) => (
                  <Link key={post.slug} to={`/substack/${post.slug}`} className={s.sideStory}>
                    ...
                    {post.imageUrl && <img src={post.imageUrl} alt="" className={s.sideThumb} />}
                  </Link>
                ))}
              </aside>
            </section>
              ...
              <section className={s.popularSection} aria-label="Most Popular">
                ...
                <div className={s.popularRail}>
                  {popularPosts.map((post) => (
                    <Link key={post.slug} to={`/substack/${post.slug}`} className={s.popularItem}>
                      <div className={s.popularText}>
                        <h3 className={s.popularTitle}>{post.title}</h3>
                        <p className={s.storyMeta}>{formatMeta(post)}</p>
                      </div>
                      {post.imageUrl ? (
                        <img src={post.imageUrl} alt="" className={s.popularThumb} />
                      ) : (
              ...
              {archivePosts.length > 0 ? (
                <div className={s.archiveGrid}>
                  {archivePosts.map((post) => (
                    <Link key={post.slug} to={`/substack/${post.slug}`} className={s.archiveCard}>
                      {post.imageUrl ? (
                        <img src={post.imageUrl} alt="" className={s.archiveImage} loading="lazy" />
                      ) : (
